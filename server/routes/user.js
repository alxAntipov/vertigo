...
userRoutes.post('/user/login', authMiddleware, (request, response) => {
  ...
  if (request.body.login) {
    User.findOne({login: request.body.login}, (error, document) => {
      ...
      if (error) {
        ...
        if (!document) {
          responseData.errors.push({type: 'warning', message: 'No user exists with this login.'})
    ...
    responseData.errors.push({type: 'critical', message: 'login not provided.'})
  ...
  if (request.body.login != '') {
    ...
    User.findOne({login: request.body.login}, (error, document) => {
      if (!document) {
        ...
        // User already exists
        responseData.errors.push({type: 'warning', message: 'The login is taken. Please choose something else.'})
        response.json(responseData)
      }
    })
  } else {
